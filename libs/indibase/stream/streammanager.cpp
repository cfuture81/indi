...
void StreamManagerPrivate::writeRecordingMetadata()
{
    if (m_currentRecordFilePath.empty())
        return;

    // Build .txt path by replacing extension
    std::string metaPath = m_currentRecordFilePath;
    auto dotPos = metaPath.rfind('.');
    if (dotPos != std::string::npos)
        metaPath = metaPath.substr(0, dotPos) + ".txt";
    else
        metaPath += ".txt";
    ...
    if (!mf)
    {
        LOGF_WARN("Could not write metadata file: %s", metaPath.c_str());
        return;
    }

    // Header
    fprintf(mf, "# Recording Metadata\n");
    fprintf(mf, "# Generated by INDI StreamManager\n\n");

    // Device info
    fprintf(mf, "Device: %s\n", getDeviceName());
    fprintf(mf, "File: %s\n\n", m_currentRecordFilePath.c_str());

    // Recording stats
    fprintf(mf, "Duration (ms): %.1f\n", FPSRecorder.totalTime());
    fprintf(mf, "Total Frames: %llu\n", (unsigned long long)FPSRecorder.totalFrames());
    fprintf(mf, "Average FPS: %.2f\n\n", FpsNP[FPS_AVERAGE].getValue());

    // Stream exposure
    fprintf(mf, "Exposure (s): %.6f\n", StreamExposureNP[STREAM_EXPOSURE].getValue());

    // Stream depth
    if (FullDepthSP[FULL_DEPTH_16BIT].getState() == ISS_ON)
        fprintf(mf, "Bit Depth: 16\n");
    else
        fprintf(mf, "Bit Depth: 8\n");

    // CCD-specific metadata
    if (currentDevice->getDriverInterface() & INDI::DefaultDevice::CCD_INTERFACE)
    {
        auto *ccd = dynamic_cast<INDI::CCD *>(currentDevice);
        if (ccd)
        {
            auto &chip = ccd->PrimaryCCD;
            fprintf(mf, "Image Width: %d\n", chip.getSubW());
            fprintf(mf, "Image Height: %d\n", chip.getSubH());
            fprintf(mf, "Pixel Size (um): %.2f x %.2f\n", chip.getPixelSizeX(), chip.getPixelSizeY());

            // Temperature (standard CCD property)
            if (ccd->HasCooler() && ccd->TemperatureNP.getState() != IPS_IDLE)
                fprintf(mf, "Temperature (C): %.1f\n", ccd->TemperatureNP[0].getValue());
        }
    }
    ...
    LOGF_INFO("Recording metadata written to %s", metaPath.c_str());
}
...
bool StreamManagerPrivate::startRecording()
{
    ...
    m_currentRecordFilePath = filename;
...
bool StreamManagerPrivate::stopRecording(bool force)
{
    ...
    // Write metadata text file alongside the recording
